...
namespace StarTrekOnline_ServerStatus
{
    ...
    public class ServerStatus : IServerStatus
    {
        ...
        {
            string monthDay = $"{times.Item1} {times.Item2}";
            string[] timeRange = times.Item3.Split('-');

            DateTime currentDate = DateTime.Now;
            int currentYear = currentDate.Year;

            DateTime monthDayDate = DateTime.ParseExact(monthDay, "MMMM d", CultureInfo.InvariantCulture);
            ...
            DateTime startEventTime = monthDayDate.Date.AddHours(startTime.Hour).AddMinutes(startTime.Minute).AddHours(8);
            DateTime endEventTime = monthDayDate.Date.AddHours(endTime.Hour).AddMinutes(endTime.Minute).AddHours(8);
